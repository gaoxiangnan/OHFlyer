...
/*  日期  */
@property (nonatomic, strong) UIImageView *showStaDImg;
@property (nonatomic, strong) UILabel *showStartDate;
@property (nonatomic, strong) UILabel *startDate;
@property (nonatomic, strong) UIImageView *showArrDImg;
@property (nonatomic, strong) UILabel *showArriveDate;
...
@implementation TripBaojiBackView
...
- (void)InitWithSubViews
{
    ...
    /*  日期  */
    [_bgView addSubview:self.showStaDImg];
    [_showStaDImg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_showStartImg);
        make.top.equalTo(_startLb.mas_bottom).offset(40);
    }];
    [_bgView addSubview:self.showStartDate];
    [_showStartDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_showStaDImg.mas_right).offset(9);
        make.top.equalTo(_showStaDImg);
    }];
    
    [_bgView addSubview:self.startDate];
    [_startDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_showStaDImg);
        make.top.equalTo(_showStaDImg.mas_bottom).offset(5);
    }];
    
    [_bgView addSubview:self.showArrDImg];
    [_showArrDImg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_bgView).offset(-81);
        make.top.equalTo(_showStaDImg);
    }];
    [_bgView addSubview:self.showArriveDate];
    [_showArriveDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_showArrDImg.mas_right).offset(5);
        make.top.equalTo(_showArrDImg);
    }];
    
    [_bgView addSubview:self.arriveDate];
    [_arriveDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_showArriveDate.mas_right);
        make.top.equalTo(_startDate);
    }];
    ...
    [_showPassengerImg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_showStaDImg);
        make.top.equalTo(_showStaDImg.mas_bottom).offset(50);
    }];
    ...
    if (kDevice_Is_iPhone5) {
        ...
        [_searchBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_bgView).offset(35);
            make.right.equalTo(_bgView.mas_right).offset(-35);
            make.top.equalTo(_bgView.mas_bottom).offset(kScaleFrom_iPhone5s_Desgin(40));
            make.height.equalTo(@(40));
        }];
    }
...
- (UIImageView *)showStaDImg
{
    if (!_showStaDImg) {
        _showStaDImg = [UIImageView new];
        _showStaDImg.image = [UIImage imageNamed:@"calender_img"];
    }
    return _showStaDImg;
}
- (UILabel *)showStartDate
{
    if (!_showStartDate) {
        _showStartDate = [UILabel new];
        _showStartDate.textColor = [UIColor darkGrayColor];
        _showStartDate.text = @"出发日期";
        _showStartDate.font = [UIFont systemFontOfSize:12 weight:UIFontWeightThin];
        _showStartDate.textAlignment = NSTextAlignmentLeft;
    }
...
- (UIImageView *)showArrDImg
{
    if (!_showArrDImg) {
        _showArrDImg = [UIImageView new];
        _showArrDImg.image = [UIImage imageNamed:@"calender_img"];
    }
    return _showArrDImg;
}
- (UILabel *)showArriveDate
{
    if (!_showArriveDate) {
        _showArriveDate = [UILabel new];
        _showArriveDate.textColor = [UIColor darkGrayColor];
        _showArriveDate.text = @"返回日期";
        _showArriveDate.font = [UIFont systemFontOfSize:12 weight:UIFontWeightThin];
        _showArriveDate.textAlignment = NSTextAlignmentLeft;
    }
    return _showArriveDate;
}
- (UILabel *)arriveDate
{
    if (!_arriveDate) {
        ...
        _arriveDate.text = @"2016-11-12 13:12";
        _arriveDate.font = [UIFont systemFontOfSize:14 weight:UIFontWeightRegular];
        _arriveDate.textAlignment = NSTextAlignmentRight;
        _arriveDate.userInteractionEnabled = YES;
        UITapGestureRecognizer *dateGes = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(onSelectDateClient)];
        [_arriveDate addGestureRecognizer:dateGes];
    }
    return _arriveDate;
}
