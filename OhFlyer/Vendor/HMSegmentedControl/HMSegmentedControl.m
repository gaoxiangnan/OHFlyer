...
#define kScaleFrom_iPhone5s_Desgin(_X_) (_X_ * (kDeviceWidth/320))
...
- (void)commonInit {
    ...
    self.selectedTextFont = [UIFont systemFontOfSize:15.0f weight:UIFontWeightRegular];
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.selectedSegmentIndex == idx) {
                ...
                titleLayer.font = (__bridge CFTypeRef _Nullable)(self.selectedTextFont);
            } else {
                titleLayer.foregroundColor = self.textColor.CGColor;
                titleLayer.font = (__bridge CFTypeRef _Nullable)(self.font);
            }
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed)
                imageXOffset = self.segmentWidth * idx + kScaleFrom_iPhone5s_Desgin(50);
            else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                NSInteger i = 0;
                for (NSNumber *width in self.segmentWidthsArray) {
